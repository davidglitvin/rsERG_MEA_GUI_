...
"""
manual_segment_eyeavg_eeg_click_popup_v17.py
...
Run from terminal:
    ...
    python manual_segment_eyeavg_eeg_click_popup_v17.py "/path/to/file_epoched.pkl" --sfreq 2000 --epoch-len 5
...
class ManualSegmentWindow(QMainWindow, QtCore.QObject):
    ...
    def _init_ui(self):
        ...
        if self.has_eeg:
            key_hint += "/'3'"
        key_hint += " switch signal, 'A' add segment (between clicks), 'U' undo (current), 'M' manage/export, 'Q' quit."

        instructions = (
            "Mouse: left-click start and end of a bad segment, then press 'A' (or click "
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python manual_segment_eyeavg_eeg_click_popup_v17.py "

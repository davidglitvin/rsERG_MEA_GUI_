...
def get_eeg_selected_column_indices(selected_channel_labels, total_channels):
    """
    Determine which *selected columns* correspond to EEG/ADC channels.

    EEG channels are assumed to be the last 8 channels of the acquisition stream.
    Returns 0-based column indices within the *selected* channel array.
    """
    try:
        candidates = set(get_eeg_candidate_labels(total_channels))
    except Exception:
        candidates = set()

    eeg_cols = []
    for col_idx, lab in enumerate(list(selected_channel_labels or [])):
        if lab in candidates:
            eeg_cols.append(int(col_idx))
    return eeg_cols
...
# S) EEG amplitude correction (for ADC/EEG channels)
adjust_eeg_amp_toggle = ToggleButtons(
    options=['No', 'Yes'],
    value='Yes',
    description='Adjust EEG amplitude?',
    button_style='',
    tooltips=[
        'Do not rescale EEG amplitude during loading',
        'Divide EEG channels by the specified factor during loading'
    ],
    disabled=True,  # Enabled only when EEG is included
    layout=widgets.Layout(width='230px')
)

eeg_divide_widget = widgets.BoundedFloatText(
    value=1000.0,
    min=0.000001,
    max=1e12,
    step=1.0,
    description='EEG divide by:',
    disabled=True,
    style={'description_width': 'initial'},
    layout=widgets.Layout(width='260px')
)

def on_adjust_eeg_amp_change(change):
    if change.get('name') == 'value':
        # Enable factor field only when both EEG and adjustment are enabled
        eeg_divide_widget.disabled = not (include_eeg_widget.value == 'Yes' and change['new'] == 'Yes')
        try:
            update_verification_window()
        except Exception:
            pass

adjust_eeg_amp_toggle.observe(on_adjust_eeg_amp_change, names='value')

def on_include_eeg_change(change):
    if change.get('name') == 'value':
        eeg_count_widget.disabled = (change['new'] != 'Yes')
        adjust_eeg_amp_toggle.disabled = (change['new'] != 'Yes')
        eeg_divide_widget.disabled = not (change['new'] == 'Yes' and adjust_eeg_amp_toggle.value == 'Yes')
...
def update_verification_window():
    ...
    with verification_output:
        ...
        # Informational: EEG amplitude adjustment status (applies only to ADC/EEG channels)
        if include_eeg_widget.value == 'Yes':
            if adjust_eeg_amp_toggle.value == 'Yes':
                try:
                    div = float(eeg_divide_widget.value)
                except Exception:
                    div = None
                eeg_cols = get_eeg_selected_column_indices(selected_channels, max_detected_channels)
                eeg_labs = [selected_channels[i] for i in eeg_cols] if eeg_cols else []
                if div is not None:
                    print(f"EEG amplitude adjustment: ON (divide by {div:g}). EEG channels detected in selection: {eeg_labs if eeg_labs else 'None'}")
                else:
                    print("EEG amplitude adjustment: ON (invalid divide factor).")
            else:
                print("EEG amplitude adjustment: OFF")
        else:
            print("EEG not included in this export.")
...
def on_process_button_clicked(b):
    ...
    with log_output:
        ...
        # EEG amplitude scaling parameters
        
        adjust_eeg_amp = (include_eeg_widget.value == 'Yes' and adjust_eeg_amp_toggle.value == 'Yes')
        
        eeg_divide_factor = None
        
        if adjust_eeg_amp:
        
            try:
        
                eeg_divide_factor = float(eeg_divide_widget.value)
        
            except Exception:
        
                eeg_divide_factor = None
        
            if eeg_divide_factor is None or (not np.isfinite(eeg_divide_factor)) or eeg_divide_factor <= 0:
        
                print("ERROR: EEG divide factor must be a finite number > 0.")
        ...
        if concatenate_selected and len(selected_paths) >= 2:
            ...
            for path in selected_paths:
                ...
                # EEG amplitude correction (ADC tail) if enabled
                if adjust_eeg_amp:
                    eeg_cols = get_eeg_selected_column_indices(selected_channels, n_channels_total)
                    if eeg_cols:
                        reshaped_data = reshaped_data.astype(np.float32, copy=False)
                        reshaped_data[:, eeg_cols] /= float(eeg_divide_factor)
...
ui = VBox([
    ...
    include_eeg_widget,
    eeg_count_widget,
    HBox([adjust_eeg_amp_toggle, eeg_divide_widget]),

...
def on_process_button_clicked(b):
    with output:
        ...
        # Debugging: Print structure of pkl_file_picker.value
        print(f"Type of pkl_file_picker.value: {type(pkl_file_picker.value)}")
        print(f"Contents of pkl_file_picker.value: {pkl_file_picker.value}")

        if not pkl_file_picker.value:
            print("Please upload a .pkl file.")
            return

        # Handle different structures: check if it's a tuple or dict
        uploaded_files = pkl_file_picker.value

        if isinstance(uploaded_files, dict):  # Expected case in older ipywidgets
            uploaded_file = next(iter(uploaded_files.values()))  # Get the first uploaded file
        elif isinstance(uploaded_files, tuple):  # Handle tuple case
            uploaded_file = uploaded_files[0]  # Access first file tuple item
        else:
            print("Unexpected structure for uploaded files. Please check ipywidgets version.")
            return

        # Ensure valid file format
        if not isinstance(uploaded_file, dict) or 'content' not in uploaded_file:
            print("Invalid file upload. Please try again.")
            return
        
        # Extract file content
        pkl_file_content = uploaded_file['content']

...
def build_exportable_plot_psd_gui():
    ...
    export_format_widget = widgets.Dropdown(options=['png', 'svg', 'jpeg', 'ppt'], value='png', description='Format:', layout=widgets.Layout(width='150px'))
    ...
    def on_load_psd_clicked(b):
        with load_output_area:
            ...
            try:
                ...
                # IMPORTANT: Sort channel keys numerically, not lexicographically
    ...
    def on_plot_psd_clicked(b):
        ...
        with plot_output_area:
            ...
            figs, kept_dict = plot_psds_with_dropped_traces(
                ...
                vertical_line_color="black",
                vertical_line_style="--",
                vertical_line_alpha=0.6  # or read from another widget if needed
            )
        ...
        with export_output_area:
            ...
            if export_fmt == 'ppt':
                ...
                for idx, fig in enumerate(current_figures, start=1):
                    ...
                    png_name = f"{fname_base}_{idx}.png"
                    fig.savefig(png_name, format='png', dpi=150)
                    left = top = Inches(1)
                    pic = slide.shapes.add_picture(png_name, left, top, Inches(8), Inches(4.5))
                    os.remove(png_name)
                ppt_file = f"{fname_base}.pptx"
                prs.save(ppt_file)
                print(f"Exported slides to {ppt_file}")
            else:
                for idx, fig in enumerate(current_figures, start=1):
    ...
    def on_export_cleaned_clicked(b):
        with export_output_area:
            ...
            try:
                ...
                print(f"Cleaned PSD saved => {out_file}")

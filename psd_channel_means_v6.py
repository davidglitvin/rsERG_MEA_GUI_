...
def build_gui():
    ...
    def update_channel_toggles():
        ...
        for channel in psds_dict.keys():
            ...
            def toggle_change(change, channel=channel):
                ...
                elif channel in deselected_channels:
    ...
    def export_callback(b):
        with output_area:
            clear_output()
            if not psds_dict:
                print("No PSD data loaded or plotted.")
                return
            fig = plt.gcf()
            export_figure(fig, export_name.value, export_format.value)

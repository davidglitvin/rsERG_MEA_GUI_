...
##############################################################################
# 3) MAIN GUI (with FileChooser for Figure/PSD Export)
##############################################################################
...
def build_exportable_plot_psd_gui():
    ...
    Adaptation of your PSD-clean GUI that includes:
      ...
      - Uses ipyfilechooser for figure export (e.g., .png, .pptx, etc.) and for cleaned PSD export (.pkl).
    """
    # ~~~~~ GUI ELEMENTS ~~~~~
    ...
    # ~~~ Export Figure (FileChooser) ~~~
    export_fig_chooser = FileChooser(
        os.getcwd(),
        title='Select or type a file name for figures (e.g. MyFigure.png or MySlides.pptx)',
        select_default=False
    )
    # You can limit or allow all: 
    ...
    # ~~~ Export Cleaned PSD (FileChooser) ~~~
    cleaned_pickle_chooser = FileChooser(
        os.getcwd(),
        title='Select or type a .pkl file name for cleaned PSD',
        select_default=False
    )
    ...
    def on_load_psd_clicked(b):
        with output_area:
            ...
            if do_segment:
                ...
                if n_blocks == 0:
                    print("No full blocks (fewer than 120 epochs). Aborting.")
                    return
        ...
        with export_output_area:
            ...
            for key, psd_info in loaded_psd.items():
                if key not in kept_indices_dict:
                    # means no dropping was performed on that key => skip or keep as is
                    continue
    ...
    ui = widgets.VBox([
        widgets.HTML("<h2>PSD Clean GUI w/Segmentation + FileChooser Exports</h2>"),

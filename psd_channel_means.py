...
def build_exportable_plot_psd_gui():
    ...
    An interactive GUI to:
      ...
      - **And** export the final "mean of means" data/figure as:
          (Pickle, Excel, PNG, SVG, JPEG, PPT)
    """
    ...
    # 12. Export controls (for subplots)
    ...
    def on_plot_psd_clicked(b):
        ...
        with plot_output_area:
            ...
            if len(selected_channels) > 16:
                print("Please select up to 16 channels.")
                return
            ...
            eye1_channels = [ch for ch in selected_channels if 1 <= int(ch[2:]) <= 8]
            eye2_channels = [ch for ch in selected_channels if 9 <= int(ch[2:]) <= 16]
            ...
            if n_subplots == 0:
                ...
                if show_eye1 and eye1_channels:
                    ...
                    plot_individual_channels(
                        ...
                        channel_means_dict=channel_means_before,  # "before"
                        freqs=freqs_e1,
    ...
    ########################################################################
    # C) EXPORT BUTTON CALLBACK (for subplots only)
    ########################################################################
        ...
        with export_output_area:
            ...
            if export_fmt == 'ppt':
                ...
                for idx, fig in enumerate(current_figures, start=1):
                    ...
                    print(f"  Saved subplot => {save_name}")

            print("Done (exported subplots).")

    export_button.on_click(on_export_button_clicked)

    ########################################################################
    # D) NEW: Export final "individual means + mean of means" data or plot
    #    in multiple formats: pickle, excel, png, svg, jpeg, or ppt
    ########################################################################
    ...
    def on_final_export_clicked(b):
        ...
        with final_export_output:
            ...
            # 1) If user wants "Pickle" => we pickle final_data_dict
            ...
            # 2) If user wants "Excel"
                ...
                try:
                    ...
                    print(f"Saved final_data_dict => {outxlsx}")
                except Exception as e:
                    print(f"ERROR saving Excel: {e}")

            # 3) If user wants image (png, svg, jpeg)
                ...
                try:
                    ...
                    print(f"Saved final_fig => {outimg}")
                except Exception as e:
            ...
            # 4) PPT
                ...
                try:
                    ...
                    os.remove(tmp_png)
                    prs.save(ppt_name)
                    print(f"Exported final_fig => {ppt_name}")
                except Exception as e:
    ...
    ui = widgets.VBox([
        widgets.HTML("<h2>Enhanced PSD Plotter GUI with Eye1/Eye2, Individual Colors, & Final Means Export</h2>"),
        
        widgets.HBox([load_psd_button, psd_file_chooser]),
        load_output_area,
        
        widgets.HBox([channels_dropdown]),

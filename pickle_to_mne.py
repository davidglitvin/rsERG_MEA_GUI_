...
def process_pickle(file_path):
    ...
    Returns:
    - epochs: MNE Epochs object containing all epochs from the pickle
    """
...
def concatenate_all_epochs(epochs_list):
    ...
    Returns:
    - combined_epochs: Single MNE Epochs object
    """
    if not epochs_list:
...
#    We’ll add a “Remove Selected” button so you can also drop them from the list:
...
# -- We add a separate button to remove highlighted items from the select widget.
...
def on_load_dir_clicked(b):
    ...
    Also, report the number of epochs in each file.
    """
    with output_area:
        clear_output()
...
def on_remove_selected_clicked(b):
    """
    Remove from pkl_select.options whatever is currently highlighted in pkl_select.value.
    """
    with output_area:
        # Read the current list of files in options
        current_options = list(pkl_select.options)
        # Files currently selected (highlighted)
        ...
        pkl_select.value = ()  # Clear the selection
...
def on_convert_button_clicked(b):
    ...
    with output_area:
        ...
        if combined_epochs:
            ...
            # Optionally, save the combined epochs or perform further processing here
            # For demonstration, we store it in a global dictionary
            global mne_combined_epochs
            mne_combined_epochs = combined_epochs
            print(f"Combined Epochs stored in 'mne_combined_epochs'.")
        else:
...
# Wire up the on_click callbacks
...
# Arrange widgets vertically and horizontally as needed
ui = VBox([

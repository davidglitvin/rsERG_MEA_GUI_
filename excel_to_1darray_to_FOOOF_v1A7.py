...
def process_excel(file_paths, include_block=True, include_eye=True):
    ...
    Only processes sheets (sub-arrays) based on the following logic:
      - If the sheet name contains a block indicator (B1, B2, ... B6), it is included only if include_block is True.
      - Otherwise, if the sheet name contains "Ey" (e.g., Ey1, Ey2) it is included only if include_eye is True.
      - Sheets that do not match either pattern are skipped.
    ...
    for file_path in file_paths:
        try:
            ...
            for sheet_name in excel_data.sheet_names:
                ...
                # Sheets with block info: contain "B" followed by a digit 1-6.
                is_block = bool(re.search(r'B[1-6]', sheet_name))
                # Sheets with "Ey" (eye) but not a block.
                ...
                if is_eye and not include_eye:
                    continue
                # If neither pattern is found, skip this sheet.
                if not is_block and not is_eye:
                    continue

                # Read the sheet and compute PSD
...
def plot_closest_to_mean(fg, sheet_name, export_dir, base_filename, formats,
                         ...
                         include_r2=False):
    """
    For FOOOFGroup objects that contain multiple PSD fits, this function
    selects 10 spectra closest to the mean and plots them.
    If export formats are provided (via 'formats'), figures are saved accordingly.
    Otherwise, the plots are simply shown.
    """
    ...
    # Optional: Prepare a PPTX if 'ppt' is among the requested formats
...
def plot_all_psds(fg, sheet_name, export_dir, base_filename, formats,
                  ...
                  include_r2=False):
    """
    For FOOOFGroup objects (even with a single PSD or many), this function plots
    each PSD (each fit) individually.
    If export formats are provided, the figures are saved accordingly.
    Otherwise, they are simply shown.
    """
    n_fits = len(fg)
    if "ppt" in formats and PPTX_AVAILABLE:
        prs = Presentation()
...
def export_fg_dict_excel(fg_dict, excel_filename):
    ...
    This updated version safely checks that parameter arrays have values.
    """
    rows = []
    for key, fg in fg_dict.items():
        n_fits = len(fg)
        # Pre-fetch parameter arrays.
...
# NEW: Options to export FOOOF results (the fg_dict) as pickle and/or Excel.
...
def on_run_button_click(b):
    ...
    with output:
        ...
        # 3. Gather figure export formats (only if exporting figures is enabled).
        ...
        if psd_data_dict_all:
            ...
            for dict_key, fg_item in fg_dict.items():
                try:
                    ...
                    # Use closest-to-mean plotting only if enabled and if more than one PSD is present.
...
display(
    widgets.VBox(
        [
            ...
            widgets.Label(value="(Optional) Grid, X-Tick Font Size, and R² in Titles:"),
            widgets.HBox([grid_checkbox, x_tick_font_size_widget, include_r2_checkbox]),

...
def compute_combined_means(channel_means, blocks, eye_channels, deselected_channels):
    ...
    for ch in eye_channels:
        ...
        for block in blocks:
            channel = f"{block}:{ch}"
            if channel in channel_means and ch not in deselected_channels:
                _, new_mean = channel_means[channel]
                valid_means.append(new_mean)
        if valid_means:
            combined_mean = np.mean(valid_means, axis=0)
            combined_means[ch] = combined_mean
...
def plot_block_layout(
    ...
    if channel_colors is None:
        ...
        # Extract unique channels without block prefixes
    ...
    # Compute combined means for both eyes
    combined_eye1 = compute_combined_means(channel_means, blocks, eye1_channels, deselected_channels)
    combined_eye2 = compute_combined_means(channel_means, blocks, eye2_channels, deselected_channels)
    ...
    for i, block in enumerate(blocks):
        ...
        for channel in eye1_channels:
            ...
            if channel in deselected_channels:
                continue
            if channel_full not in channel_means:
                continue
...
def build_gui():
    ...
    def update_channel_toggles():
        ...
        # Extract unique channel names without block prefixes
        unique_channels = sorted({key.split(':Ch')[-1] for key in psds_dict.keys()})
        toggles = []
    ...
    def export_callback(b):
        with output_area:
            ...
            try:
                ...
                ppt = None  # Initialize PPT presentation if needed

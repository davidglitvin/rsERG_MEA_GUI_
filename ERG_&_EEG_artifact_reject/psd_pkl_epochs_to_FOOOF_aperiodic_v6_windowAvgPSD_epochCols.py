        ...
        with export_output:
            ...
            for key, ap in aperiodic_summary.items():
                ...
                for w_idx in range(n_windows):
                    ...
                    # Derived fields for easier plotting/reconstruction in Excel
                    try:
                        anchor_val = row.get("anchor_epoch", np.nan)
                        if not np.isnan(anchor_val):
                            row["anchor_epoch_1based"] = int(anchor_val) + 1
                        else:
                            row["anchor_epoch_1based"] = np.nan
                    except Exception:
                        row["anchor_epoch_1based"] = np.nan

                    # Approximate total number of epochs contributing to this key
                    n_epochs_full = np.nan
                    try:
                        if win_end is not None and hasattr(win_end, "__len__") and win_end.size > 0:
                            n_epochs_full = int(np.nanmax(win_end))
                    except Exception:
                        n_epochs_full = np.nan
                    row["n_epochs_full"] = n_epochs_full

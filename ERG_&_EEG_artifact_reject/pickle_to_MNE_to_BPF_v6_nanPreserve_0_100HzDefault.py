...
def _nan_aware_filter_2d(
    ...
    for ch in range(n_channels):
        ...
        # find contiguous segments in finite_idx
        starts = [finite_idx[0]]
        ends = []
        for i in range(1, finite_idx.size):
...
def nan_aware_filter_epochs(
    ...
    """Apply NaN-preserving band-pass filter to an Epochs object.

    The underlying data (n_epochs, n_channels, n_times) is flattened to
    (n_channels, n_epochs*n_times), filtered with _nan_aware_filter_2d,
    then reshaped back. NaNs are preserved in their original locations.
    """
    data = epochs.get_data(copy=True)  # shape: (n_epochs, n_channels, n_times)
    n_epochs, n_channels, n_times = data.shape
    sfreq = float(epochs.info["sfreq"])

    # Flatten epochs -> continuous
    data_flat = data.transpose(1, 0, 2).reshape(n_channels, n_epochs * n_times)

    # NaN-aware filtering
    data_flat_filt = _nan_aware_filter_2d(
        data_flat,
        sfreq=sfreq,
        l_freq=l_freq,
        h_freq=h_freq,
    ...
    data_filt = data_flat_filt.reshape(n_channels, n_epochs, n_times).transpose(1, 0, 2)

    # Recreate Epochs with same metadata
...
# NOTE: In this v4 'nanPreserve' version, NaNs are *never* dropped or interpolated.
# They are preserved in-place, and band-pass filtering is applied only to contiguous
# finite segments between NaNs (see nan_aware_filter_epochs).
nan_policy_widget = widgets.Dropdown(
    options=[
        ('Preserve NaNs (NaN-aware filter; fixed)', 'preserve'),
    ],
    value='preserve',
    description='NaN policy:',
    layout=widgets.Layout(width='380px'),
    disabled=True,
    tooltip='NaNs are preserved in-place; filtering skips NaN segments (segment-wise).'
)

# Dummy widget kept for layout compatibility; hidden by default.
nan_fill_value_widget = widgets.FloatText(
    value=0.0,
    description='Fill value:',
    layout=widgets.Layout(width='220px'),
    tooltip='Not used in nanPreserve mode.'
)
...
def process_and_filter_files(file_list):
    ...
    for pkl_file in tqdm(file_list, desc="Processing files"):
        ...
        # --- Apply Bandpass Filter (NaN-preserving) ---
        try:
            epochs_filtered = nan_aware_filter_epochs(
                epochs,
                l_freq=l_freq,
                h_freq=h_freq,
                fir_design=fir_design,
...
def process_pickle(
    ...
    This version PRESERVES NaNs and does not drop/fill/interpolate them.
    NaNs are carried through into the MNE Epochs and are handled later
    by `nan_aware_filter_epochs` during band-pass filtering.
    ...
    sfreq_fallback : float
        Sampling frequency to use if not found in the pickle metadata.
    ch_name_mode : str
        Passed to `infer_ch_names` if no channel names are present in the pickle.
    prefer_pickle_ch_names : bool
        If True and valid ch_names are present in the pickle, they are used.
    nan_policy : str
        Kept for backward compatibility but IGNORED in this NaN-preserving version.
    nan_fill_value : float
        Unused here; included only for API compatibility.
    ...
    if kind == "blocks":
        ...
        all_epochs = []
        for block_name in sorted(payload.keys()):
            block_data = np.asarray(payload[block_name], dtype=float)
            # NaNs will be preserved and handled later by nan_aware_filter_epochs.
            n_nonfinite = np.sum(~np.isfinite(block_data))
            if n_nonfinite > 0:
                print(
                    f"  Found {n_nonfinite} non-finite values (NaN/Inf) in block '{block_name}'. "
                    f"These will be preserved during filtering."
                )
            if block_data.ndim != 3:
                print(f"  SKIP block '{block_name}': expected 3D array, got shape {block_data.shape}")
                continue
            print(f"  Processing block: {block_name} with shape {block_data.shape}")
            ch_names = _resolve_names(block_data.shape[1])
            epochs_block = create_epochs_array(block_data, sfreq=sfreq_use, ch_names=ch_names)
            all_epochs.append(epochs_block)
        if all_epochs:
            combined_epochs = mne.concatenate_epochs(all_epochs)
            return combined_epochs
        else:
    ...
    if n_nonfinite > 0:
        print(
            f"Non-finite values detected in {os.path.basename(file_path)}: {n_nonfinite}. "
            f"These will be preserved during filtering."
        )

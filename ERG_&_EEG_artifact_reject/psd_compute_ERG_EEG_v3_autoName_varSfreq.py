...
# -------------------------------------------------------------------------
# Channel naming helper for Eye1 / Eye2 / EEG layouts
# -------------------------------------------------------------------------

def apply_eye_eeg_channel_naming(epochs):
    """Rename channels by index for ERG/EEG hybrid recordings.

    Assumptions:
      - Channels 1–8  -> Eye1
      - Channels 9–16 -> Eye2
      - Channel 17    -> EEG
      - Any channels beyond 17 will be named EEG_2, EEG_3, ...

    The function preserves the underlying data order and only updates labels.
    """
    ...
    for i in range(0, min(8, n)):
        rename_map[ch_names[i]] = f"Eye1_Ch{i+1}"

    # Eye2
    for i in range(8, min(16, n)):
        rename_map[ch_names[i]] = f"Eye2_Ch{i-7}"

    # EEG
    if n >= 17:
        rename_map[ch_names[16]] = "EEG"
        if n > 17:
            eeg_counter = 2
            for i in range(17, n):
                rename_map[ch_names[i]] = f"EEG_{eeg_counter}"
                eeg_counter += 1

    epochs = epochs.copy()
    epochs.rename_channels(rename_map)

    # Best-effort channel type assignment
    try:
        type_map = {}
        if n >= 17:
            type_map["EEG"] = "eeg"
        for new_name in rename_map.values():
            if new_name.startswith("EEG_"):
                type_map[new_name] = "eeg"
        if type_map:
            epochs.set_channel_types(type_map)
    except Exception:
        pass
...
# 2B. Optional channel auto-naming for Eye1/Eye2/EEG layouts
auto_name_widget = widgets.Checkbox(
    value=True,
    description='Auto-name Eye1/Eye2/EEG channels',
    indent=False,
    tooltip='Rename ch1-8 -> Eye1_Ch1-8, ch9-16 -> Eye2_Ch1-8, ch17 -> EEG'
)

# 2C. Optional sampling-rate override (useful if metadata is missing or incorrect)
override_sfreq_enable_widget = widgets.Checkbox(
    value=False,
    description='Override sfreq',
    indent=False,
    tooltip='Use a custom sampling rate instead of the value stored in the .fif file'
)

override_sfreq_widget = widgets.FloatText(
    value=10000.0,
    description='sfreq (Hz):',
    layout=widgets.Layout(width='180px'),
    disabled=True,
    tooltip='Custom sampling rate in Hz'
)

def _on_override_sfreq_toggle(change):
    if change.get('name') == 'value':
        override_sfreq_widget.disabled = not bool(change['new'])

override_sfreq_enable_widget.observe(_on_override_sfreq_toggle, names='value')
...
def on_compute_psd_clicked(b):
    ...
    with psd_output_area:
        ...
        # 2.5 Optional: apply Eye1/Eye2/EEG channel naming by index
        if auto_name_widget.value:
            try:
                epochs = apply_eye_eeg_channel_naming(epochs)
                print("Applied Eye1/Eye2/EEG channel naming by index.")
                print(f"Updated channel names: {epochs.ch_names}\n")
            except Exception as e:
                print(f"WARNING: Failed to apply Eye/EEG naming: {e}\n")
        ...
        sfreq = float(epochs.info.get('sfreq', np.nan))
        if override_sfreq_enable_widget.value:
            try:
                sfreq = float(override_sfreq_widget.value)
            except Exception:
                pass
        if not np.isfinite(sfreq) or sfreq <= 0:
            print("ERROR: Sampling rate (sfreq) is missing or invalid. ")
            print("       Use 'Override sfreq' to set it manually.")
            return

        nperseg = int(round(window_length_s * sfreq))
        nperseg = max(nperseg, 1)
        noverlap = int(round(nperseg * (overlap_percent / 100.0)))
        if noverlap >= nperseg:
            noverlap = max(0, nperseg - 1)
        
        # Validate nperseg against epoch length
        n_times = epochs.get_data().shape[2]
        epoch_len_s_calc = n_times / sfreq
        print(f"Detected sfreq: {sfreq} Hz")
        print(f"Epoch length: {epoch_len_s_calc:.3f} s ({n_times} samples)\n")
        if window_length_s > epoch_len_s_calc:
            print(f"ERROR: Window length ({window_length_s}s) exceeds epoch length ({epoch_len_s_calc:.3f}s).")
            return
        if nperseg > n_times:
...
psd_params_box = widgets.HBox([
    ...
    overlap_widget,
    auto_name_widget,
    override_sfreq_enable_widget,
    override_sfreq_widget
])

...
def compute_welch_psd(
    ...
    """
    Compute Welch PSD per-channel from an MNE Epochs object.

    NaN/Inf handling is performed per-channel on the (n_epochs, n_times) matrix
    *before* Welch, to avoid non-finite values propagating into PSDs.

    Returns
    -------
    out_dict : dict
        {channel_name: {"freqs": freqs, "psd": psd}} where psd has shape
        (n_epochs_kept, n_freqs_selected). If a channel has no usable epochs,
        returns {"freqs": None, "psd": None}.
    """
    out_dict = {}
    ...
    # One-time summary (per compute call)
    ...
    if nonfinite_total > 0:
        ...
        print(f"[NaN handling] Detected {nonfinite_total} non-finite samples across {bad_epochs_any}/{n_ep} epochs. Policy='{nan_policy}'.")
        if nan_policy in ("fill_constant", "interpolate", "fill_median"):
            print(f"[NaN handling] Non-dropping policy; nan_fill_value={nan_fill_value}")

    for ch_idx, ch_name in enumerate(epochs.ch_names):
        ch_data = data[:, ch_idx, :]  # (n_ep, n_time)

        cleaned_ch, keep_mask, stats = _clean_channel_matrix_nonfinite(
            ch_data,
            policy=nan_policy,
            fill_value=nan_fill_value,
        )

        # Optional channel-level note when dropping
        if stats.get("n_nonfinite", 0) > 0 and nan_policy == "drop_epochs":
            if stats.get("n_dropped_epochs", 0) > 0:
                print(f"[NaN handling] {ch_name}: dropped {stats['n_dropped_epochs']} epochs with NaN/Inf.")

        if cleaned_ch.size == 0 or cleaned_ch.shape[0] == 0:
...
def build_exportable_plot_psd_gui():
    ...
    nan_policy_widget = widgets.Dropdown(
        options=[
            ('Drop affected epochs (per channel)', 'drop_epochs'),
            ('Interpolate NaNs/Infs (linear)', 'interpolate'),
            ('Fill NaNs/Infs with constant', 'fill_constant'),
            ('Fill NaNs/Infs with per-epoch median', 'fill_median'),

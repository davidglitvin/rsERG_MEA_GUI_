...
def _classify_eye_group(ch_name):
    ...
    return None
def _classify_eye_eeg_group(ch_name):
    """Classify a channel name into 'Eye1', 'Eye2', 'EEG', or None.

    Rules:
        - Strip any 'blockX:' prefix before classification.
        - If name (lowercased) contains 'eye1' -> 'Eye1'
        - If name contains 'eye2' -> 'Eye2'
        - If name contains 'eeg'  -> 'EEG'
...
def build_exportable_plot_psd_gui():
    ...
    eeg_keys_global = []  # EEG-classified channels among selected keys
    ...
    def on_plot_psd_clicked(b):
        nonlocal current_figures, final_data_dict, final_fig, eeg_keys_global
        with plot_output_area:
            ...
            # Determine Eye1/Eye2/EEG groups based on channel names
            eye1_keys = []
            eye2_keys = []
            eeg_keys = []
            for ch in selected_keys:
                cls = _classify_eye_eeg_group(ch)
                if cls == "Eye1":
                    eye1_keys.append(ch)
                elif cls == "Eye2":
                    eye2_keys.append(ch)
                    ...
                    eeg_keys.append(ch)
                        ...
                        # Heuristic for 3-channel Eye1/Eye2/EEG datasets:
            # if everything was classified as Eye1 and there are exactly 3
            # selected keys (e.g., Ch1/Ch2/Ch3 or unlabeled Eye1/Eye2/EEG
            # averages), reinterpret them as Eye1/Eye2/EEG by order.
            if eye1_keys and not eye2_keys and not eeg_keys and len(selected_keys) == 3:
                print(
                    "Heuristic applied: 3-channel dataset with no explicit Eye2/EEG labels. "
                    "Treating first as Eye1, second as Eye2, third as EEG."
                )
                # Preserve original ordering from the selection widget
                ordered = list(selected_keys)
                eye1_keys = [ordered[0]]
                eye2_keys = [ordered[1]]
                eeg_keys = [ordered[2]]

# Store EEG keys in shared state for later use (final export, etc.)
            eeg_keys_global = eeg_keys
            ...
            def on_final_button_clicked(bb):
                nonlocal final_data_dict, final_fig, current_figures, eeg_keys_global
                with out_final:
                    clear_output()
                    ...
                    # EEG group: use all channels that were classified as EEG among the selected keys
                    final_eeg = list(eeg_keys_global) if eeg_keys_global else []
                    ...
                    group_mean_eeg  = compute_group_mean(channel_means_after, final_eeg) if final_eeg else None
                    ...
                    # 5) "Means only" figure: Eye1 vs Eye2 (and EEG) curves
                    if (group_mean_eye1 is None) and (group_mean_eye2 is None) and (group_mean_eeg is None):
                    ...
                    if group_mean_eeg is not None and final_eeg:
                        freqs_eeg = loaded_psd[final_eeg[0]]["freqs"]
                        ax_mom.plot(freqs_eeg, group_mean_eeg, color="green", linewidth=2, label="EEG group mean")
                        final_data_dict["EEG_MeanOfMeans"] = group_mean_eeg
                        final_data_dict["EEG_freqs"] = freqs_eeg

                    ax_mom.set_title("Final Eye1/Eye2/EEG means of means", fontsize=title_fs)
        ...
        with final_export_output:
            ...
            if ext == ".pkl":
                ...
                try:
                    ...
                    # Determine a reference frequency axis
                    freq_arr = None
                    for fk in ("Eye1_freqs", "Eye2_freqs", "EEG_freqs"):
                        if fk in final_data_dict and final_data_dict[fk] is not None:
                            freq_arr = np.asarray(final_data_dict[fk]).ravel()
                            break

                    if freq_arr is None:
                        print("No frequency axis ('Eye1_freqs', 'Eye2_freqs', 'EEG_freqs') found in final_data_dict; cannot export Excel.")
                        return

                    n = freq_arr.shape[0]

                    def _prepare(col_key):
                        arr = final_data_dict.get(col_key)
                        if arr is None:
                            return np.full(n, np.nan, dtype=float)
                        arr = np.asarray(arr, dtype=float).ravel()
                        if arr.shape[0] != n:
                            print(f"Warning: {col_key} length {arr.shape[0]} != freq length {n}; truncating/padding.")
                            if arr.shape[0] > n:
                                arr = arr[:n]
                            else:
                    ...
                    eye1 = _prepare("Eye1_MeanOfMeans")
                    eye2 = _prepare("Eye2_MeanOfMeans")
                    eeg  = _prepare("EEG_MeanOfMeans")

                    df = pd.DataFrame(
                        {
                            "Freq_Hz": freq_arr,
                            "Eye1_MeanOfMeans": eye1,
                            "Eye2_MeanOfMeans": eye2,
                            "EEG_MeanOfMeans": eeg,
                        }
                    )
                    df.to_excel(path, index=False)
                    print(f"Saved Eye1/Eye2/EEG means to Excel => {path}")
                except Exception as e:

...
def plot_closest_to_mean(fg, sheet_name, export_dir, base_filename, formats,
    ...
    Applies user-defined x/y limits (common axis) and removes axis padding.
    Forces integer tick marks for the x-axis from x_min to x_max in steps of 1 Hz.
    ...
    for i, index in enumerate(closest_indices):
        ...
        # ---- Apply the user-defined axis limits and remove padding ----
        plt.xlim(x_min, x_max)
        plt.ylim(y_min, y_max)
        # Force integer freq ticks from floor(x_min) to ceil(x_max)
        x_min_int = int(np.floor(x_min))
        x_max_int = int(np.ceil(x_max))
        plt.xticks(range(x_min_int, x_max_int + 1, 1))

        # Remove any auto-axis padding
        plt.margins(0)
